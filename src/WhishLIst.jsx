        ...
        <div className="min-h-[100vh] mb-[100px] w-full xl:w-full md:w-full sm:pl-0 lg:pl-0 md:pl-0 pl-[100px] xl:pl-0">
          <div className="h-[100%] sm:w-full relative top-[50px] xl:w-[100%] xl:min-h-screen sm:top-8 flex flex-wrap sm:flex-wrap md:flex md:flex-wrap md:justify-center xl:justify-center lg:justify-center justify-start pl-[20px] sm:pl-0 md:pl-0 xl:pl-0 sm:items-center sm:justify-center items-start rounded-3xl w-full gap-4 md:gap-[8px] lg:gap-8 xl:gap-8 sm:gap-2">
            {wishlist.map((product, id) => (
              <div key={id}
                className="h-[85%] w-[265px] sm:w-[170px] sm:h-[265px] md:w-[45%] md:h-[375px] rounded-3xl flex flex-col items-center sm:flex-col md:flex-col md:items-center md:justify-center md:mb-[10px] sm:mb-[10px] sm:ml-[2px]">
                <div className="h-auto w-[265px] sm:w-full md:w-[270px] md:h-[350px] sm:h-full bg-[#e4cfcf] shadow-xl rounded-3xl flex flex-col items-start justify-start shrink-0">
                  <Link to={`/products/${product.id}`} className="w-full h-full flex flex-col items-center justify-center sm:z-[998] ">
                    ...
                    <div className="h-[120px] sm:h-[51%] w-full sm:w-[90%] flex flex-col items-start justify-around sm:justify-around sm:rounded-3xl pl-7 sm:pl-0">
                      <div className="h-[150px] relative top-0 mb-0 sm:top-2 left-1 sm:left-0 text-left w-[90%] sm:w-[100%] sm:pl-[5px] sm:h-[20px] pr-1 sm:text-balance leading-0 flex items-center sm:items-start justify-start">
                        <p className="text-[17px] sm:text-[14px] sm:font-normal sm:w-[110px] md:text-[20px] sm:leading-[16px] text-[#000] font-bold capitalize overflow-hidden">
                          {product.title}
                        </p>
                      </div>
                    </div>
                  </Link>
                  <div className="h-full w-full z-[998] sm:w-full md:w-full sm:h-[25%] flex justify-end items-center relative bottom-[40px] sm:bottom-[35px] md:bottom-[40px] ">
                    <FaTrash onClick={() => removeWishlistItem(id)}
                      className="relative right-[50px] sm:right-[18px] md:right-[50px] md:text-[30px] bottom-[5px]  hover:text-[#866b6b] transition duration-400 ease sm:text-[23px] text-[30px] text-red-700" />
                  </div>
                </div>
              </div>
            ))}
